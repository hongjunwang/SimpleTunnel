...
open class Tunnel: NSObject {
    ...
    /// The current set of logical connections open with the tunnel.
    var connections = [Int: Connection]()
    
    /// The list of data that needs to be writtern to the tunnel connection when possible.
    ...
    func closeTunnel() {
        ...
        if let index = Tunnel.allTunnels.index(where: { $0 == self }) {
    ...
    class func closeAll() {
        ...
        allTunnels.removeAll(keepingCapacity: false)
    }
    ...
    func sendMessage(_ messageProperties: [String: AnyObject]) -> Bool {
        ...
        if savedData.isEmpty {
            // There is nothing queued up to be send, to directly write to the tunnel.
            written = writeDataToTunnel(messageData, startingAtOffset: 0)
            if written < 0 {
                closeTunnel()  // May be there exist a bug
            }
        }
    ...
    func sendData(_ data: Data, forConnection connectionIdentifier: Int) {
        ...
        if !sendMessage(properties) {
            simpleTunnelLog("Failed to send a data mesaage for connection \(connectionIdentifier)")
        }
    }
    
    /// Send a Data mesaage with an associated endpoint.
    ...
    func sendCloseType(_ type: TunnelConnectionCloseDirection, forConnection connectionIdentifier: Int) {
