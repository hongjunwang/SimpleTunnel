...
class ServerTunnelConnection: Connection {
	...
	func readPackets() {
        ...
        let utunSocket = source.handle

		repeat {
			let readCount = readv(Int32(utunSocket), iovecListPointer.baseAddress, Int32(iovecListPointer.count))
	...
	override func sendPackets(_ packets: [Data], protocols: [NSNumber]) {
		...
		for (index, packet) in packets.enumerated() {
			...
			let writeCount = writev(Int32(utunSocket), &iovecList, Int32(iovecList.count))
			if writeCount < 0 {
