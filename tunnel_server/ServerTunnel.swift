...
class ServerTunnel: Tunnel, TunnelDelegate, StreamDelegate {
	...
	class func startListeningOnPort(_ port: Int32) -> NetService {
        ...
        service.schedule(in: RunLoop.main, forMode: .defaultRunLoopMode)
	...
	func sendOpenResultForConnection(_ connectionIdentifier: Int, resultCode: TunnelConnectionOpenResult) {
	...
	override func handleMessage(_ commandType: TunnelCommand, properties: [String: AnyObject], connection: Connection?) -> Bool {
		switch commandType {
			...
			case .fetchConfiguration:
				...
				let messageProperties = createMessagePropertiesForConnection(0, commandType: .fetchConfiguration, extraProperties: [
                    TunnelMessageKey.Configuration.rawValue: personalized as AnyObject])
